...
def annotatorinfo():
	...
	try:
		resp_df = pd.read_csv(os.path.join(outdir, "entire_responses.csv")) # if available, open the response df in read mode 
		...
		empty.to_csv(os.path.join(outdir, "entire_responses.csv"), index=False) 
		resp_df = pd.read_csv(os.path.join(outdir, "entire_responses.csv")) 
...
def play_audio():
    ...
    global row
    global audiofile
    idx = ((resp_df.shape[0])*2)+1 # clips already listened to
    row = df.loc[idx] # select one row
...
def next_audio():
    ...
    print(beginoptions, adult_tc, adult_oc, adult_others, adult_unsure, OC_tc, OC_oc, OC_adults, OC_unsure, media, childvoc, childsleep, adultfemale, adultmale, adults, unsure, PID, annotate_date_YYYYMMDD, content, clip_comments) 

    global row
    global resp_df
    allcols = pd.DataFrame([row]).assign(beginoptions=beginoptions, Adult2TargetChild=adult_tc, Adult2OtherChild=adult_oc,  Adultfemale=adultfemale, Adultmale=adultmale, Multiple_adults=adults, Unsure_adults=unsure, Adult2Others=adult_others, Adult2unsure=adult_unsure, Otherchild2TargetChild=OC_tc, Otherchild2OtherChild=OC_oc, Otherchild2adults=OC_adults, Otherchild2unsure=OC_unsure, Media=media, Childvoc=childvoc, Childsleep=childsleep, PID=PID, comments=clip_comments, annotate_date_YYYYMMDD=annotate_date_YYYYMMDD, annotator=content, repeats=repeat_ct) 
    resp_df = pd.concat([resp_df, allcols], sort=True)
    resp_df.to_csv(os.path.join(outdir, "entire_responses.csv"), index=False)  # yes, this overwrites responses.csv each time  

    global idx 
    idx += 2 # update the global idx
...
def main():
	...
	tk.Label(frame, text="Is the child sleeping?").grid(row=21, column=0)
	childsleepcat = tk.IntVar()
	tk.Checkbutton(frame, text='Yes', variable=childsleepcat).grid(row=21, column=1)





	tk.Label(frame, font=fontStyle, text="PID").grid(row=23, column=0)
	tk.Label(frame, text="Is there PID in the clip?").grid(row=25, column=0)
	PIDcat = tk.IntVar()
	tk.Checkbutton(frame, text='Yes', variable=PIDcat).grid(row=25, column=1)

	


	tk.Label(frame, font=fontStyle, text="Comments about clip?").grid(row=27, column=0)
	comments = tk.Entry(frame) 
	comments.grid(row=27, column=1, columnspan=2)
