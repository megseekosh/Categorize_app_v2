...
que_sim_stats <- que_sim_results %>%
  ...
  mutate(sim_stat = paste(mean_sim_que,"(",sd_sim_que,")")) %>%
  select(sim_stat)
...
span_sim_stats <- span_sim_results %>%
  ...
  mutate(sim_stat = paste(mean_sim_span,"(",sd_sim_span,")")) %>%
  select(sim_stat)
...
span_sim_child_stats <- span_sim_results %>%
  ...
  summarize(mean_sim_span = round(mean(percen_span),2),
         sd_sim_span = round(sd(percen_span),2)) %>%
  mutate(sim_stat_child = paste(mean_sim_span,"(",sd_sim_span,")")) %>%
...
knitr::kable(lang_tbl, caption = 'Average minority language estimates by corpus and annotation method.', 
             booktabs=T, 
             row.names = FALSE, 
             col.names = c("Corpus (language)", "Random", "All-day", "Avg. relative error (SD)", "n=100 simulations of random sampling", "Correlation between estimates")) %>% 
...
cds_sim_stats <- cds_sim_results %>%
  ...
  mutate(sim_stat = paste(mean_sim_cds,"(",sd_sim_cds,")")) %>%
  select(id, sim_stat)
...
ads_sim_stats <- ads_sim_results %>%
  ...
  mutate(sim_stat_ads = paste(mean_sim_ads,"(",sd_sim_ads,")")) %>%
...
knitr::kable(final_reg_tbl2, caption = 'Target child-directed speech estimates by child and annotation method.', 
             booktabs=T, 
             row.names = FALSE, 
             col.names = c("Corpus (ID)", "Random", "All-day", "Relative error", "n=100 simulations of random sampling", "Within-corpus correlation between random and all-day estimates")) %>% # "
...
knitr::kable(final_reg_tbl, caption = 'Average adult-directed speech estimates by corpus and annotation method.', 
             booktabs=T, 
             row.names = FALSE, 
             col.names = c("Corpus", "Random", "All-day", "Average relative error (SD)", "n=100 simulations of random sampling",  "Correlation between estimates")) %>% 
...
knitr::kable(ques_tbl, caption = 'Spanish language estimates in U.S. corpus, by child and estimation method.', 
             booktabs=T, 
             row.names = FALSE, 
             col.names = c("Child ID", "Random", "All-day", "n=100 simulations of random sampling", "Parental Questionnaire")) %>% 
...
cdsspan_sim_stats <- cdsspan_sim_results %>%
  ...
  mutate(sim_stat_cdsspan = paste(mean_sim_cdsspan,"(",sd_sim_cdsspan,")")) %>%
