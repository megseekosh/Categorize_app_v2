...
\CommentTok{#cds_model_data_full <- cds_rolling_complete2 %>% # all the data from the 'complete' annotation}
\CommentTok{#  group_by(id) %>%}
\CommentTok{#  arrange(annotation_num)}
...
\CommentTok{# current not doing this as we're not fitting models to the 'complete' dataset}
...
\CommentTok{#span_model_data_full <- span_rolling_complete2 %>% # Bolivia & US; all the data from the 'all-day' annotation}
\CommentTok{#  group_by(id) %>%}
\CommentTok{#  arrange(annotation_num)}
